#Thi src is private don't share
import os
import sys
import time
import json
...
GUEST_UID = "4279609024"
GUEST_PASSWORD = "299A7C9BA0D06D9026BD210ECAE45439BD4290CD2815C4FB5E384C33D1EF38C4"
region = 'IN'
            ...
            while True:
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        insquad = True
                        try:
                            welcome_message = """[B][C][FFFFFF]Welcome to Jubayer Bot
...
[B][C][FFFFFF]Telegram : @mr-ghost_34
[B][C][FFFFFF]Contact : @jubayer_ahmed_34
...
[B][C][FFFFFF]Status: Online & Active"""
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = """[B][C][FFFFFF]Welcome to Jubayer Bot
...
[FFFFFF]• itachi • dreamspace • eclipse
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 3:
                                ...
                                await dl_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                try:
                                    success = await Room_Spam(target_uid, room_id, "Jubayer CODEX", key, iv)
                                    if success:
                        ...
                        if inPuTMsG.strip().startswith('/sticker'):
                            packet = await send_sticker(uid, chat_id, key, iv, nickname="JubayerxBOT")
                            if packet:
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                ...
                                await dl_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.strip().lower() == "Jubayer bhai mera uid do":
...
[FFFFFF]├──────────┤
[FFFFFF] Developer  → JUBAYER
[FFFFFF] Status     → ONLINE
[FFFFFF] Version    → OB53 V1
[FFFFFF]└───────────┘"""
...
async def MaiiiinE():
    ...
    try:
        ...
        if not Uid or not Pw:
            print("❌ UID or Password missing in bot.txt")
            return None

    except Exception as e:
        print("❌ Error reading bot.txt:", e)
        return None
    ...
    data2 = await DecRypTLoGinDaTa(LoGinDaTa)

    OnLineIP, OnLinePort = data2.Online_IP_Port.split(":")
    ChatIP, ChatPort = data2.AccountIP_Port.split(":")
    ...
    task1 = asyncio.create_task(
        TcPChaT(ChatIP, ChatPort, await xAuThSTarTuP(int(TarGeT), token, int(timestamp), key, iv),
                key, iv, data2, ready_event, region)
    )

    task2 = asyncio.create_task(
        TcPOnLine(OnLineIP, OnLinePort, key, iv,
                  await xAuThSTarTuP(int(TarGeT), token, int(timestamp), key, iv))
    )
