...
region = 'IN'
WHITELISTED_UIDS = {
    "Ꭻuʙꫝყᴇʀ", # don't change this text
    "5791471936"
}
# ADMIN INFO FUNCTION FOR ADMIN COMMAND 
ADMIN_UID = "5791471936"
server2 = "BD"
key2 = "mg24"
...
uid=4283772118,password=D15639C9FAD5641AAEA0D063DD84D634A8DD59C3FAA1F490D7CF6C6CE60DA09D

# OR Format 2: Line-separated
# uid: 4283772118
# password: D15639C9FAD5641AAEA0D063DD84D634A8DD59C3FAA1F490D7CF6C6CE60DA09D
...
def get_player_info(uid):
    try:
        url = f"https://info-silk-ten.vercel.app/get?uid={uid}"
...
async def handle_join_req_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type, LoGinDaTaUncRypTinG):
    ...
    if len(parts) < 2:
        error_msg = f"""[B][C][FF00FF]❌ Usage: /join_req (player_uid)
...
"""
    ...
    if not target_uid.isdigit():
        error_msg = f"[B][C][FF00FF]❌ Invalid UID! Must be numbers only.\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
...
"""
            ...
            success_msg = f"""[B][C][FF00FF]❌ FAILED!
...
"""
        ...
        error_msg = f"[B][C][FF00FF]❌ Error: {str(e)[:50]}\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)        
...
"""
            ...
            success_msg = f"""[B][C][FF00FF]❌ FAILED TO START MATCH!
    ...
    try:
        ...
        fields = {
            ...
            2: {
                ...
                6: "iG:[C][B][FF00FF] @hn_gaming99",  # Nickname
                7: 330,                   # Rank
                8: 1000,                  # Field 8
...
async def handle_badge_command(cmd, inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    parts = inPuTMsG.strip().split()
    if len(parts) < 2:
        error_msg = f"[B][C][FF00FF]❌ Usage: /{cmd} (uid)\nExample: /{cmd} 123456789\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
    ...
    if not target_uid.isdigit():
        error_msg = f"[B][C][FF00FF]❌ Please write a valid player ID!\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
    ...
    try:
        ...
        if badge_packet:
            ...
            success_msg = f"[B][C][FF00FF]❌ Failed to create badge packet!\n"
        
        await safe_send_message(chat_type, success_msg, uid, chat_id, key, iv)
        
    except Exception as e:
        error_msg = f"[B][C][FF00FF]❌ Error in /{cmd}: {str(e)}\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
...
async def handle_reject_completion(spam_task, target_uid, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        error_msg = f"[B][C][FF00FF]❌ ERROR in reject spam: {str(e)}\n"
...
async def black666(client_id, key, iv):
    banner_text = "[FF00FF][B][C] ERROR , WELCOME TO [FFFFFF]—͞JUBAYERᅠ [00FF00]___ —͞JUBAYERᅠ____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF00FF]TELEGRAM : @Mr.ghost34\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
async def get_colorful_message(message_text, message_number):
    ...
    color_palette = ["FF00FF", "00FF00", "0000FF", "FFFF00", "FF00FF", 
                     "FF00FF", "FFA500", "FF1493", "00FF7F", "7B68EE",
...
async def handle_msg_spam_completion(spam_task, message_text, times, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        if actual_times >= times:
            ...
            completion_msg = f"[B][C][FF00FF]❌ MESSAGE SPAM FAILED!\n"
            completion_msg += f"[FFFFFF]📝 Message: {message_text}\n"
            completion_msg += f"[FFFFFF]📊 Requested: {times} times\n"
            completion_msg += f"[FFFFFF]❌ Sent: 0 times\n"
            completion_msg += f"[FF00FF]✗ Failed to send any messages\n"
        ...
        error_msg = f"[B][C][FF00FF]❌ ERROR in message spam completion: {str(e)}\n"
        await safe_send_message(chat_type, error_msg, sender_uid, chat_id, key, iv)
...
async def handle_room_message_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) < 3:
        error_msg = f"""[B][C][FF00FF]❌ Usage: /roommsg (room_id) (message)
...
"""
    ...
    if not room_id.isdigit():
        error_msg = f"[B][C][FF00FF]❌ Room ID must be numbers only!\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        print(error_msg)
        return
...
"""
            ...
            success_msg = f"[B][C][FF00FF]❌ Failed to create room packet!\n"
...
async def handle_training_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) == 1:
        ...
        if success:
            ...
            success_msg = f"[B][C][FF00FF]❌ Failed to start training!\n"
        ...
        if success:
            ...
            success_msg = f"[B][C][FF00FF]❌ Failed to start custom training!\n"
            
        await safe_send_message(chat_type, success_msg, uid, chat_id, key, iv)
        
    else:
        error_msg = f"[B][C][FF00FF]❌ Usage: /train [custom]\nExamples:\n/train - Start default training\n/train custom - Custom training\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
...
def send_tiktok_info(username):
    ...
    try:
        response = requests.get(
            f"https://tiktok-info-mg24-pro.vercel.app/tiktok?username={username}",
            timeout=15
        )

        if response.status_code != 200:
            return f"[B][C][FF00FF]❌ TikTok API Error! Status Code: {response.status_code}"

        user = response.json()

        if user.get("credit") != "—͞JUBAYERᅠ":
            return "[B][C][FF00FF]❌ User Not Found Or Credit Invalid!"
...
"""
        ...
        return "[B][C][FF00FF]❌ TikTok API Connection Failed!"
    except Exception as e:
        return f"[B][C][FF00FF]❌ Unexpected Error: {str(e)}"
...
async def send_youtube_info(channel_name, chat_type, uid, chat_id, key, iv):
    ...
    # Main info message
    main_info = f"""
[B][C][FF00FF]╭[FF00FF]─[FF00FF]╮[FFFFFF]
[C][B][FF00FF]│[FFFFFF]▶[FF00FF] │[FFFFFF]║[00BFFF]YOUTUBE INFO[FFFFFF]║
[C][B][FF00FF]╰[FF00FF]─[FF00FF]╯[FFFFFF]
[C][B][FF00FF]━━━━━━━━━━━
...
[C][B][FF00FF]━━━━━━━━━━━
[C][B][FFFFFF]Developer     : —͞JUBAYERᅠ
"""
    # Send main info
...
"""
        ...
        return "[B][C][FF00FF]❌ Guild API Connection Failed!"
    except Exception as e:
        return f"[B][C][FF00FF]❌ Unexpected Error: {str(e)}"
...
[FFD700]NAME        : [{rare_color}]{data.get('name', 'N/A')}
...
def get_math_result(input_expr):
    ...
    url = f"https://math-api-mg24-pro.vercel.app/math?expression={encoded_expr}"
    ...
    try:
        ...
        if data.get("status") != "success":
            ...
            return f"""[B]
[FF00FF]═════════════
[FF00FF]        INVALID EXPRESSION
[FF00FF]═════════════
[FF4444]EXPRESSION : [FFFFFF]{xMsGFixinG(expression)}
[FF00FF]RESULT     : [FFFFFF]ERROR    
[FF00FF]═════════════
""".strip()
...
[FF00FF]        MATH RESULT
[FF1493]═════════════
[FFD700]EXPRESSION : [FFFFFF]{xMsGFixinG(expression)}    
...
def send_likes(uid):
    try:
        ...
        likes_api_response = requests.get(
             f"https://mahir-like-web.vercel.app/like?uid={uid}&server_name=bd",
             timeout=15
        )
        ...
        # Key-গুলো JSON অনুযায়ী ঠিক করা হয়েছে (S বড় হাতের)
        player_name = api_json_response.get('Nickname', 'Unknown')
        likes_before = api_json_response.get('Before', 0)
        likes_after = api_json_response.get('After', 0)
        likes_added = api_json_response.get('Given', 0)
        status = api_json_response.get('Status', 0)  # এখানে 'S' বড় হাতের হবে
...
[C][B][11EAFD]‎━━━━━━━━━━━━
"""
        # যদি আগে থেকেই নেওয়া থাকে বা লিমিট শেষ হয়
        else:
            return f"""
[C][B][FF00FF]━━━━━━━━━━━━
...
def fake_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://fake-like-mg24-pro.vercel.app/like?uid={uid}&server_name=bd",
             timeout=15
             )
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]—͞JUBAYERᅠ [00FF00]!!
"""
...
[FF00FF]You have already taken likes with this UID.
Try again after 24 hours.

[FFFFFF]Player Name : [FF00FF]{xMsGFixinG(player_name)}  
[FFFFFF]Likes Before : [FF00FF]{xMsGFixinG(likes_before)}  
[FFFFFF]Likes After : [FF00FF]{xMsGFixinG(likes_after)}  
[C][B][FF00FF]━━━━━━━━━━━━
"""
        else:
            # ❓ Unexpected case
...
def talk_with_ai(question):
    url = f"https://ai-api.vercel.app/ask?key=MG24_GAMER&question={question}"
    res = requests.get(url)
    if res.status_code == 200:
        data = res.json()
...
def spam_requests(player_id):
    # This URL now correctly points to the Flask app you provided
    url = f"https://mg24-gamer-spam-api.vercel.app/spam?uid={player_id}&region=bd"
    try:
        res = requests.get(url, timeout=20) # Added a timeout
        if res.status_code == 200:
            data = res.json()
...
def get_random_color():
    colors = [
        "[FF00FF]", "[00FF00]", "[0000FF]", "[FFFF00]", "[FF00FF]", "[FF00FF]", "[FFFFFF]", "[FFA500]",
...
"""
            ...
            if leave_success:
                leave_msg = f"[B][C][FF00FF]🚪 Bot left team to restart cycle...\n"
...
    
[00FF00]ＷＥＬＣＯＭＥ ＴＯ[FF00FF] M G 2 4  G A M E R   [00FF00]ＢＯＴ
[FF00FF]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[FF00FF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[1E90FF]ＤＥＶ   [FF00FF]M G 2 4  G A M E R  
[FF00FF]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[FF00FF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF00FF]M G 2 4  G A M E R  
[FF00FF]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[FF00FF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[9400D3]M A D E B Y [FF00FF]M G 2 4   G A M E R
[FF00FF]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[FF00FF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TIKTOK: [FF00FF]@9x_jubayer_
[FF00FF]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[FF00FF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="RIJEXX"):
    ...
    try:
        ...
        nested_details.Nickname = f"[C][B][FF00FF]{nickname}"  # CHANGE: Your nickname
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="—͞JUBAYERᅠ", title_id=title_id)
...
"""
        ...
        error_msg = f"[B][C][FF00FF]❌ Error sending titles: {str(e)}\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
...
async def handle_all_titles_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type=0):
    ...
    if len(parts) == 1:
        ...
        error_msg = f"""[B][C][FF00FF]❌ Usage: /alltitles [uid]
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde —͞JUBAYERᅠ ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="—͞JUBAYERᅠ", title_id=title_id)
...
async def handle_xjoin_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) < 3:
        error_msg = f"""[B][C][FF00FF]🎮 ROOM JOIN COMMAND
...
"""
            ...
            success_msg = f"[B][C][FF00FF]❌ Failed to create room join packet!\n"
        
        await safe_send_message(chat_type, success_msg, uid, chat_id, key, iv)
        
    except Exception as e:
        error_msg = f"[B][C][FF00FF]❌ Error joining room: {str(e)}\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
...
async def handle_room_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) < 2:
        error_msg = f"[B][C][FF00FF]❌ Usage: /room (uid)\nExample: /room 14444444004\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
        return
    ...
    try:
        ...
        if not status_result:
            ...
            if cached_data and 'packet' in cached_data:
                ...
                error_msg = f"[B][C][FF00FF]❌ Player {xMsGFixinG(target_uid)} not found\n"
                await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
                return
        else:
            # Use live data
...
💡 Player must join custom room first!"""
        ...
        if not room_id:
            error_msg = f"[B][C][FF00FF]❌ Failed to extract room ID\n"
            await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
            return
...
"""
        ...
        error_msg = f"[B][C][FF00FF]❌ Error: {str(e)[:80]}\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
...
async def handle_room_spam_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    if len(parts) < 4:
        error_msg = f"""[B][C][FF00FF]❌ Usage: /spamroom (room_id) (uid) (message)
...
"""
    ...
    try:
        ...
        if not target_uid.isdigit():
            error_msg = f"[B][C][FF00FF]❌ UID must be numbers only!\n"
            await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
            return
...
"""
            ...
            success_msg = f"[B][C][FF00FF]❌ Failed to create spam packet!\n"
        
        await safe_send_message(chat_type, success_msg, uid, chat_id, key, iv)
        
    except Exception as e:
        error_msg = f"[B][C][FF00FF]❌ Error: {str(e)}\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)
...
async def handle_freeze_completion(freeze_task, uid, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        completion_msg = f"""[B][C][FF00FF]❄️ FREEZE COMMAND COMPLETED!
...
"""
        ...
        error_msg = f"[B][C][FF00FF]❌ Freeze error: {str(e)}\n"
        await safe_send_message(chat_type, error_msg, sender_uid, chat_id, key, iv)
...
"""
            ...
            success_msg = f"""[B][C][FF00FF]❌ GENERATION FAILED!
...
"""
        ...
        error_msg = f"[B][C][FF00FF]❌ Generation error: {str(e)[:50]}\n"
        await safe_send_message(chat_type, error_msg, uid, chat_id, key, iv)        
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        if "Ꭻuʙꫝყᴇʀ" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 13777711848
                                message_text = f" Can't accept Your request Talk to —͞JUBAYERᅠ"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF00FF]█▓▒░░ WELCOME TO —͞JUBAYERᅠ BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF00FF] —͞JUBAYERᅠ
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFFF00]@jubayer_ahmed_34
[00FF00]»»————-　★　————-««
[FF00FF]█▓▒░░ ENJOY THE BOT! ░░▒▓█"""
...
"""
                                ...
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Please provide a question after /ai\nExample: /ai What is Free Fire?\n"
                        ...
                        if inPuTMsG.strip().startswith('/freeze'):
                            ...
                            if len(parts) < 2:
                                error_msg = f"""[B][C][FF00FF]❄️ FREEZE COMMAND
...
"""
                                ...
                                # Send initial message
                                initial_msg = f"""[B][C][FF00FF]❄️ FREEZE COMMAND STARTING!
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/bio'):
                            ...
                            if len(parts) < 2:
                                error_msg = f"""[B][C][FF00FF]❌ Usage: /bio (your bio text)

📝 Examples:
/bio Hello World!
/bio 🤖 Bot by —͞JUBAYERᅠ
/bio Level 70 | Pro Player
/bio Add me: —͞JUBAYERᅠ
...
"""
                                ...
                                # Check length
                                if len(bio_text) > 50:
                                    error_msg = f"[B][C][FF00FF]❌ Bio too long! Max 50 characters.\n📝 Your bio: {len(bio_text)} chars\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
                                ...
                                if not credentials:
                                    error_msg = f"[B][C][FF00FF]❌ Failed to load credentials from file!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
                                ...
                                if not Uid or not Pw:
                                    error_msg = f"[B][C][FF00FF]❌ Invalid credentials format!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
...
"""
                                    ...
                                    success_msg = f"""[B][C][FF00FF]❌ BIO UPDATE FAILED AFTER {max_retries} ATTEMPTS!
...
"""
                            ...
                            if len(parts) < 3:
                                ...
                                try:
                                    ...
                                    if success:
                                        ...
                                        success_message = f"[B][C][FF00FF]❌ Regular attack failed: {result}\n"
                        ...
                        if inPuTMsG.strip().startswith('/inv'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /inv (uid)\nExample: /inv 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                try:
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ ERROR sending invite: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/like'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /like <uid>\nExample: /like 436🤫856🤫97🤫33\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nSending Likes...\n"
                        ...
                        if inPuTMsG.strip().startswith('/item'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /item <item_id>\nExample: /item 909🤫042🤫00🤫7\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                item_id = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nFetching Item Info...\n"
                        ...
                        if inPuTMsG.strip().startswith('/math'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /math <question>\nExample: /math 2+3\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                expression = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nSolving Calculation...\n"
                        ...
                        if inPuTMsG.strip().startswith('/spam_req'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /spam_req <uid>\nExample: /spam_req 436🤫856🤫97🤫33\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/visit'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /visit <uid>\nExample: /visit 436🤫856🤫97🤫33\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nSending Visit...\n"
                        ...
                        if inPuTMsG.strip().startswith('/tt'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /tt <username>\nExample: /tt virat.kohli\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
...
# yt info command handler   
                        if inPuTMsG.strip().startswith('/yt'):  
                            ...
                            if not target_channel:  
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /yt <channel>\nExample: /yt mr.ghost-34\n"  
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)  
                            else:  
...
# GUILD INFORMATION FF
                        if inPuTMsG.strip().startswith('/guild'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /guild <guild_id>\nExample: /guild 308🤫431🤫816🤫6\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                guild_id = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/check'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /check <uid>\nExample: /check 436🤫856🤫97🤫33\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nChecking Ban Status...\n"
                        ...
                        if inPuTMsG.strip().startswith('/wlremove'):
                            ...
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ Usage: /wlremove (uid)\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
                            ...
                            if str(response.Data.uid) != "14444444004":
                                error_msg = f"[B][C][FF00FF]❌ Only bot owner can remove from whitelist!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
                            ...
                            if success:
                                ...
                                result_msg = f"[B][C][FF00FF]❌ {message}\n"
                        ...
                        if inPuTMsG.strip().startswith('/wladd'):
                            ...
                            if len(parts) < 2:
                                error_msg = f"""[B][C][FF00FF]❌ Usage: /wladd (uid)
...
"""
                            ...
                            if str(response.Data.uid) != "14444444004":  # Replace with your actual UID
                                error_msg = f"[B][C][FF00FF]❌ Only bot owner can add to whitelist!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
...
"""
                                ...
                                success_msg = f"[B][C][FF00FF]❌ {message}\n"
                        ...
                        if inPuTMsG.strip() == '/wllist':
                            ...
                            if str(response.Data.uid) != "14444444004":  # Your UID
                                error_msg = f"[B][C][FF00FF]❌ Only bot owner can view whitelist!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
                        ...
                        if inPuTMsG.strip() == '/viewguests':
                            ...
                            try:
                                if not os.path.exists("guest_accounts.json"):
                                    error_msg = f"[B][C][FF00FF]❌ No guest accounts found!\n[FFFFFF]Generate with /guest (count) first\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/guest'):
                            ...
                            if len(parts) < 2:
                                error_msg = f"""[B][C][FF00FF]❌ Usage: /guest (count)
...
"""
                            ...
                            if not count_input.isdigit():
                                error_msg = f"[B][C][FF00FF]❌ Count must be a number!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
                            ...
                            if count <= 0:
                                error_msg = f"[B][C][FF00FF]❌ Count must be greater than 0!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
    
                            if count > 100:
                                error_msg = f"[B][C][FF00FF]❌ Max 100 accounts at once!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
...
"""
                            ...
                            try:
                                ...
                                error_msg = f"[B][C][FF00FF]❌ Error starting generation: {str(e)[:50]}\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            
                        if inPuTMsG.startswith('/mimic_on'):
                            success_msg = f"[B][C][FF00FF]The Mimic Is Now OFF\n"
                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            emote_hijack = True
                        ...
                        if inPuTMsG.strip().startswith('/dm '):
                            ...
                            if len(parts) < 3:
                                error_msg = f"""[B][C][FF00FF]❌ Usage: /dm (target_uid) (message)
...
"""
                            ...
                            if not target_uid.isdigit() or len(target_uid) < 8:
                                error_msg = f"[B][C][FF00FF]❌ Invalid UID! Must be 8+ digits\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
    
                            # Validate message length
                            if len(message_text) > 100:
                                error_msg = f"[B][C][FF00FF]❌ Message too long! Max 100 characters\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
...
"""
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ Failed to create message packet!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/room_msg'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /kick (uid)\nExample: /kick 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_id = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/add'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /add <uid>\nExample: /add 436🤫856🤫97🤫33\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nSending Requests...\n"
                        ...
                        if inPuTMsG.strip().startswith('/remove'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /remove <uid>\nExample: /remove 436🤫856🤫97🤫33\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\n Removing Requests...\n"
                        ...
                        if inPuTMsG.strip().startswith('/joinroom'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF00FF]❌ Usage: /joinroom (room_id) (password)\nExample: /joinroom 123456 0000\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_id = parts[1]
                                ...
                                try:
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ Failed to join room: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == "/admin":
                            ...
                            admin_message = """
[B][C][FF0050]সাদিয়া,[FFFFFF]  
[C][00F2EA]তুমি আমার হৃদয়ের সবচেয়ে আপন মানুষ।  
[C][FFB703]তোমাকে ভালোবাসি শব্দে বোঝানো যায় না—  
[C][FF0050]তুমি আমার পৃথিবী। ❤️

[B][C][00F2EA]ღ সাদিয়া ღ  
[C][FF0050]তুমি আমার হৃদয়ের রানী 👑  
[C][FFFFFF]ভালোবাসি তোমায় সীমাহীন—  
[C][00F2EA]তুমিই আমার সবকিছু 💞
"""
                        ...
                        if inPuTMsG.strip().startswith('/multijoin'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ Usage: /multijoin (target_uid)\nExample: /multijoin 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
...
"""
                                        ...
                                        result_msg = f"[B][C][FF00FF]❌ All join requests failed! Check bot connection.\n"
            
                                    await safe_send_message(response.Data.chat_type, result_msg, uid, chat_id, key, iv)
            
                                except Exception as e:
                                    error_msg = f"[B][C][FF00FF]❌ Multi-join error: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/reject'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /reject (target_uid)\nExample: /reject 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                        ...
                        if inPuTMsG.strip() == '/reject_stop':
                            if reject_spam_task and not reject_spam_task.done():
                                ...
                                error_msg = f"[B][C][FF00FF]❌ No active reject spam to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/info'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /info <uid>\nExample: /info 436🤫856🤫97🤫33\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nGetting Player Info...\n"
                        ...
                        if inPuTMsG.strip().startswith('/createroom'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF00FF]❌ Usage: /createroom (room_name) (password) [players=4]\nExample: /createroom BOTROOM 0000 4\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_name = parts[1]
                                room_password = parts[2]
                                ...
                                try:
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ Failed to create room: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)               
                        ...
                        if inPuTMsG.strip().startswith('/arr'):
                            ...
                            if len(parts) < 2:
                                error_msg = f"""[B][C][FF00FF]❌ Usage: /entry (uid)
                        ...
                        """
                                ...
                                try:
                                    ...
                                    if entry_packet:
                                        ...
                                        error_msg = f"[B][C][FF00FF]❌ Failed to create entry emote packet!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                
                                except Exception as e:
                                    error_msg = f"[B][C][FF00FF]❌ Error sending entry emote: {str(e)[:50]}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.startswith('/join'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /join (team_code)\nExample: /join ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        if ghost_packet:
                                            ...
                                            error_msg = f"[B][C][FF00FF]❌ ERROR! Failed to create ghost join packet.\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                    
                                    except Exception as ghost_error:
                                        print(f"Ghost join also failed: {ghost_error}")
                                        error_msg = f"[B][C][FF00FF]❌ ERROR! Failed to join squad: {str(ghost_error)}\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/ghost'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /ghost (team_code)\nExample: /ghost ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                CodE = parts[1]
                                ...
                                try:
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ ERROR! Ghost join failed: {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
...
# NEW LAG COMMAND (AUTO STOP AFTER 10 SECONDS)
                        if inPuTMsG.strip().startswith('/lag '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /lag (team_code)\nExample: /lag ABC123\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
...
# NEW ATTACK COMMAND (AUTO STOP AFTER 1 SECOND)
                        if inPuTMsG.strip().startswith('/attack '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /attack (target)\nExample: /attack TEST\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/mg'):
                            ...
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ Usage: /mg (message) [repeats=5]\n"
                                ...
                                try:
                                    ...
                                    if repeats <= 0:
                                        error_msg = f"[B][C][FF00FF]❌ Repeats must be > 0!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    elif repeats > 10:
                                        error_msg = f"[B][C][FF00FF]❌ Max 10 repeats!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    elif len(message_text) < 2:
                                        error_msg = f"[B][C][FF00FF]❌ Message must be at least 2 characters!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    else:
                                        global mg_spam_task
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ Invalid format!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/msg'):
                            ...
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /msg (message) (times)\n"
                                ...
                                try:
                                    ...
                                    if times <= 0:
                                        error_msg = f"[B][C][FF00FF]❌ ERROR! Times must be greater than 0!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    
                                    elif not message_text.strip():
                                        error_msg = f"[B][C][FF00FF]❌ ERROR! Message cannot be empty!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    else:
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ ERROR! Invalid format!\n"
                        ...
                        if inPuTMsG.strip() == '/stop msg':
                            if msg_spam_task and not msg_spam_task.done():
                                ...
                                error_msg = f"[B][C][FF00FF]❌ No active message spam to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/e'):
                            ...
                            # Parse command
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /e [emote_name_or_number]\n"
                            ...
                            try:
                                ...
                                if is_number or is_name:
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ Invalid emote: '{last_part}'\n"
                                ...
                                if not emote_id:
                                    error_msg = f"[B][C][FF00FF]❌ Emote '{emote_name_display}' not found!\n"
                                ...
                                if success_count > 0:
                                    ...
                                    if failed_uids:
                                        success_msg += f"[FF00FF]❌ Failed: {', '.join(failed_uids)}\n"
            
                                    success_msg += f"[FFFFFF]────────────────\n"
            
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                else:
                                    error_msg = f"[B][C][FF00FF]❌ Failed to send emote to any target!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/me'):
                            ...
                            # Check usage
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /me [team_code] [emote] or /me [team_code] [uid] [emote]\n"
                                error_msg += f"[FFFFFF]Examples:\n"
                            ...
                            try:
                                ...
                                if not emote_id:
                                    error_msg = f"[B][C][FF00FF]❌ Invalid emote: '{emote_key}'\nUse /e list names to see all available names\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    for target_uid in target_uids:
                                        H = await Emote_k(target_uid, int(emote_id), key, iv, region)
                                ...
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Failed to execute /me command: {str(e)[:50]}\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
...
# GLOBAL BLOCKED NAMES
                        ...
                        if inPuTMsG.strip().startswith('/gali'):
                            ...
                            try:
                                ...
                                if blocked:
                                    block_msg = "[B][C][FF00FF]❌ This name is protected and cannot be targeted!"
                                ...
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Something went wrong:\n{str(e)}"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
...
# PRAISA COMMAND (17 POSITIVE MESSAGES)
                        ...
                        if inPuTMsG.strip().startswith('/fast'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /fast uid1 [uid2] [uid3] [uid4] emoteid\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not emote_id:
                                    error_msg = f"[B][C][FF00FF]❌ ERROR! Invalid format! Usage: /fast uid1 [uid2] [uid3] [uid4] emoteid\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                        ...
                        if inPuTMsG.strip().startswith('/p'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 4:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /p (uid) (emote_id) (times)\nExample: /p 123456789 909000001 10\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                try:
                                    target_uid = parts[1]
                                    ...
                                    if times <= 0:
                                        ...
                                        error_msg = f"[B][C][FF00FF]❌ ERROR! Maximum 100 times allowed for safety!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    else:
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ ERROR! Invalid number format! Usage: /p (uid) (emote_id) (times)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                except Exception as e:
                                    error_msg = f"[B][C][FF00FF]❌ ERROR! {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/spam '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ Usage: /spam (uid)\nExample: /spam 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                try:
                                    ...
                                    if not os.path.exists("vv.json"):
                                        error_msg = f"[B][C][FF00FF]❌ ERROR: vv.json file not found!\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        return
                                except:
                                    pass
...
"""
                                    ...
                                    import traceback
                                    traceback.print_exc()
                                    error_msg = f"[B][C][FF00FF]❌ SPAM ERROR: {str(e)[:50]}...\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)      
                        ...
                        if inPuTMsG.strip().startswith('/spm_inv'):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ Usage: /spm_inv (uid)\nExample: /spm_inv 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                        ...
                        if inPuTMsG.strip() == '/stop spm_inv':
                            if spam_request_task and not spam_request_task.done():
                                ...
                                error_msg = f"[B][C][FF00FF]❌ ERROR! No active spam request to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/status '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ Usage: /status (player_uid)\nExample: /status 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
                            ...
                            try:
                                ...
                                status_packet = await createpacketinfo(target_uid, key, iv)
                                if not status_packet:
                                    error_msg = f"[B][C][FF00FF]❌ Failed to create status packet!\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    return
                                ...
                                if not response_received:
                                    ...
                                    error_msg = f"[B][C][FF00FF]❌ STATUS CHECK FAILED\n"
                                    error_msg += f"────────────────\n"
                        ...
                        if inPuTMsG.strip().startswith('/evo '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /evo uid1 [uid2] [uid3] [uid4] number(1-21)\nExample: /evo 123456789 1\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not number:
                                    error_msg = f"[B][C][FF00FF]❌ ERROR! Invalid format! Usage: /evo uid1 [uid2] [uid3] [uid4] number(1-21)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                                        if number_int not in EMOTE_MAP:
                                            error_msg = f"[B][C][FF00FF]❌ ERROR! Number must be between 1-21 only!\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                                            ...
                                            if success:
                                                ...
                                                error_msg = f"[B][C][FF00FF]❌ ERROR! {result_msg}\n"
                                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                            
                                    except ValueError:
                                        error_msg = f"[B][C][FF00FF]❌ ERROR! Invalid number format! Use 1-21 only.\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/evo_fast '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /evo_fast uid1 [uid2] [uid3] [uid4] number(1-21)\nExample: /evo_fast 123456789 1\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                # Parse uids and number
                                uids = []
                                ...
                                if not uids or not number:
                                    error_msg = f"[B][C][FF00FF]❌ ERROR! Invalid format! Usage: /evo_fast uid1 [uid2] [uid3] [uid4] number(1-21)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                        ...
                        if inPuTMsG.strip().startswith('/evo_c '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF00FF]❌ ERROR! Usage: /evo_c uid1 [uid2] [uid3] [uid4] number(1-21) time(1-100)\nExample: /evo_c 123456789 1 10\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                ...
                                if not uids or not number or not time_val:
                                    error_msg = f"[B][C][FF00FF]❌ ERROR! Invalid format! Usage: /evo_c uid1 [uid2] [uid3] [uid4] number(1-21) time(1-100)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        number_int = int(number)
                                        ...
                                        if number_int not in EMOTE_MAP:
                                            ...
                                            error_msg = f"[B][C][FF00FF]❌ ERROR! Time must be between 1-100 only!\n"
                                            await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        else:
                                        ...
                                        error_msg = f"[B][C][FF00FF]❌ ERROR! Invalid number/time format! Use numbers only.\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop evo_fast':
                            if evo_fast_spam_task and not evo_fast_spam_task.done():
                                ...
                                error_msg = f"[B][C][FF00FF]❌ ERROR! No active evolution fast spam to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                ...
                                error_msg = f"[B][C][FF00FF]❌ ERROR! No active evolution custom spam to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
...
#==================≈===========  /HELP MENU COMMANDS ========================================
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "hello", "hi"):
                            ...
                            header = f"[C][B][FF1493]Hey User Welcome To [00FFAA]—͞JUBAYERᅠ's BOT"
                            await safe_send_message(response.Data.chat_type, header, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu1 = """[C][B][FFD700]═══⚡COMMAND MENU 1-8⚡═══[00FFFF][B]
├─ [00FFFF]⚡Help Menu 
│  └─ [FF00FF]/help
...
│  └─ [FF00FF]/6
├─ [00FFFF]Join Bot in Team
│  └─ [FF00FF]/join [team_code]
├─ [00FFFF]Leave Bot Team
│  └─ [FF00FF]/exit
├─ [00FFFF]Kick Player From Team
│  └─ [FF00FF]/kick [uid]
└─ [00FFFF]Start Match 
   └─ [FF00FF]/start 
[00FFFF]━━━━━━━━━━━━[FF00FF]"""
                            ...
                            menu2 = """[C][B][FFD700]═══⚡COMMAND MENU 9-16⚡═══[00FFFF][B]
├─ [00FFFF]Send Likes To Account⚡
│  └─ [FF00FF]/like [uid]
├─ [00FFFF]Player Information 
│  └─ [FF00FF]/info [uid]
├─ [00FFFF]Get Player Bio
│  └─ [FF00FF]/bio [uid]
├─ [00FFFF]Account Level Info
│  └─ [FF00FF]/level [uid]
├─ [00FFFF]TikTok Id Info
│  └─ [FF00FF]/tt [username]
├─ [00FFFF]YouTube Channel Info
│  └─ [FF00FF]/yt [name]
├─ [00FFFF]Guild Information 
│  └─ [FF00FF]/guild [guild_id]
└─ [00FFFF]Get Player Status 
   └─ [FF00FF]/status [uid]
[00FFFF]━━━━━━━━━━━━[FF00FF]"""
                            await safe_send_message(response.Data.chat_type, menu2, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu3 = """[C][B][FFD700]═══⚡COMMAND MENU 17-24⚡═══[00FFFF][B]
├─ [00FFFF]Chat With Ai
│  └─ [FF00FF]/ai [question]
├─ [00FFFF]Account Ban Check 
│  └─ [FF00FF]/check [uid]
├─ [00FFFF]Add Bot In Friend List 
│  └─ [FF00FF]/add [uid]
├─ [00FFFF]Remove From Friend List 
│  └─ [FF00FF]/remove [uid]
├─ [00FFFF]Gali Friend 
│  └─ [FF00FF]/gali [name]
├─ [00FFFF]Direct Message 
│  └─ [FF00FF]/dm [uid] [msg]
├─ [00FFFF]Message Spam 
│  └─ [FF00FF]/msg [msg] [repeat]
└─ [00FFFF]Wave Messages 
   └─ [FF00FF]/mg [msg] [repeat]
[00FFFF]━━━━━━━━━━━━[FF00FF]"""
                            await safe_send_message(response.Data.chat_type, menu3, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu4 = """[C][B][FFD700]═══⚡COMMAND MENU 25-32⚡═══[00FFFF][B]
├─ [00FFFF]Bundle List
│  └─ [FF00FF]/bundle 
├─ [00FFFF]Set Bundle 
│  └─ [FF00FF]/bundle [name]
├─ [00FFFF]Lag Team 
│  └─ [FF00FF]/lag [Team Code]
├─ [00FFFF]Attack Team 
│  └─ [FF00FF]/attack [Team Code]
├─ [00FFFF]Level Up Bot
│  └─ [FF00FF]/lw [Team Code]
├─ [00FFFF]Stop Level Up Bot 
│  └─ [FF00FF]/stop
├─ [00FFFF]Developer Info
│  └─ [FF00FF]/dev
└─ [00FFFF]Admin Info
   └─ [FF00FF]/admin
[00FFFF]━━━━━━━━━━━━[00FFFF]"""
                            ...
                            menu5 = """[C][B][FFD700]═══⚡COMMAND MENU 33-38⚡═══[00FFFF][B]
├─ [00FFFF]Bundle Spam 
│  └─ [FF00FF]/bb_lag [Team Code]
├─ [00FFFF]Tcp Store
│  └─ [FF00FF]/store
├─ [00FFFF]Spam Invite 
│  └─ [FF00FF]/spm_inv [uid]
├─ [00FFFF]Spam Friend Requests 
│  └─ [FF00FF]/spam_req [uid]
├─ [00FFFF]Spam Join Requests 
│  └─ [FF00FF]/spam_join [uid]
├─ [00FFFF]Join Req With Badge 'C'
│  └─ [FF00FF]/s1 [uid]
├─ [00FFFF]Join Req With Badge 'V'
│  └─ [FF00FF]/s2 [uid]
└─ [00FFFF]Join Req With Badge 'FF'
   └─ [FF00FF]/s3 [uid]
[00FFFF]━━━━━━━━━━━━[00FFFF]"""
                            await safe_send_message(response.Data.chat_type, menu5, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu6 = """[C][B][FFD700]═══⚡COMMAND MENU 39-46⚡═══[00FFFF][B]
├─ [00FFFF]Join Req With Badge 'Old V'
│  └─ [FF00FF]/s4 [uid]
├─ [00FFFF]Join Req With Badge 'Pro'
│  └─ [FF00FF]/s5 [uid]
├─ [00FFFF]Bot Speed ⚡⚡⚡
│  └─ [FF00FF]/speed
├─ [00FFFF]Evo Cycle Self 
│  └─ [FF00FF]@evos
├─ [00FFFF]Evo Cycle Player 
│  └─ [FF00FF]@evos [uid]
├─ [00FFFF]Stop Evo Cycle 
│  └─ [FF00FF]@sevos
├─ [00FFFF]Play Emote Self
│  └─ [FF00FF]/e [emote]
└─ [00FFFF]Play Emote Player 
   └─ [FF00FF]/e [uid] [emote]
[00FFFF]━━━━━━━━━━━━[FF00FF]"""
                            await safe_send_message(response.Data.chat_type, menu6, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu7 = """[C][B][FFD700]═══⚡COMMAND MENU 47-54⚡═══[00FFFF][B]
├─ [00FFFF]Play Emote Without Bot
│  └─ [FF00FF]/me [Team Code] [emote]
├─ [00FFFF]Player Emote Without Bot
│  └─ [FF00FF]/me [tc] [uid] [emote]
├─ [00FFFF]Play Evo Emote 
│  └─ [FF00FF]/evo [uid] [1-21]
├─ [00FFFF]Play Fast Emote 25x
│  └─ [FF00FF]/fast [uid] [emote id]
├─ [00FFFF]Off/Mute Bot (admin)
│  └─ [FF00FF]/off
├─ [00FFFF]On/Unmute Bot (admin)
│  └─ [FF00FF]/unmute 
├─ [00FFFF]Block User (admin)
│  └─ [FF00FF]/block [uid]
└─ [00FFFF]Unblock User (admin)
   └─ [FF00FF]/unblock [uid]
[00FFFF]━━━━━━━━━━━━[00FFFF]"""
                            await safe_send_message(response.Data.chat_type, menu7, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            menu8 = """[C][B][FFD700]═══⚡COMMAND MENU 55-62⚡═══[00FFFF][B]
├─ [00FFFF]Good Message To Player 
│  └─ [FF00FF]/praisa [name]
├─ [00FFFF]Send Fake Likes⚡
│  └─ [FF00FF]/fake_like [uid]
├─ [00FFFF]Item Id To Info
│  └─ [FF00FF]/item [item_id]
├─ [00FFFF]Super Calculator 
│  └─ [FF00FF]/math [expression / 2+3]
├─ [00FFFF]Coming Soon...
│  └─ [FF00FF]/soon... [null]
├─ [00FFFF]Coming Soon...
│  └─ [FF00FF]/soon... [null]
...
[FF00FF]Auto Accept Team Invite
[FF00FF]Auto Reply 'noob'
[FF00FF]Auto Play Ring After Join
[FF00FF]Auto Ring For Inviter
[FF00FF]/sticker & /title Work In Only Team Chat
[FF00FF]If Any Problems, 
[FF00FF]Dm Owner In Tiktok: @9x_jubayer 
[FF00FF]━━━━━━━━━━━━"""
